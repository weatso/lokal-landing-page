...
export default function ValetIndonesia() {
  const defaultValetPricing = {
    ...
    promoCodes: [
      { id: 'p1', code: 'VALET2024', discount: 25, isActive: true }
    ]
  }
  const [pricingData, setPricingData] = useState(defaultValetPricing)
    ...
    if (saved) {
      try {
        ...
        if (parsed['valet-indonesia']) {
          const data = parsed['valet-indonesia']
          if (!data.promoCodes) data.promoCodes = defaultValetPricing.promoCodes
          setPricingData(data)
        }
      } catch (e) {
  ...
  const handleConsultation = (paket = '') => {
    const paketText = paket ? `paket ${paket}` : 'sistem Valet Indonesia'
    const promoText = appliedPromoCode ? ` (Kode Promo: ${appliedPromoCode}, Diskon ${promoDiscount}%)` : ''
    const msg = `Halo LOKAL, saya tertarik dengan ${paketText}${promoText}. Mohon info cara berlangganan dan pembayarannya.`
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] font-sans text-[#333333] flex flex-col selection:bg-[#E8681A] selection:text-white">
      <SEO 
        title="LOKAL x Valet Indonesia | Parkir & Valet Cepat dengan AI" 
        description="Platform SaaS operasional valet dan parkir dengan AI scan plat. Dapatkan diskon eksklusif dan layanan prioritas."
      />
      <Navbar />
      ...
      <main className="flex-grow pt-24 pb-16">
        ...
        <section className="px-4 mb-24 relative">
          ...
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
            <div className="text-left">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="bg-white p-2 rounded-lg border border-gray-100 shadow-sm">
                  <img src="/logo-produk/valet-indonesia.png" alt="Valet Indonesia" className="h-8 object-contain" />
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <a 
                  href="https://valetindonesia.com/demo/?demo_role=admin"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-white text-indigo-600 border-2 border-indigo-100 px-8 py-4 rounded-xl font-bold hover:bg-indigo-50 transition flex items-center justify-center"
                >
                  Coba Demo Live
                </a>
              </div>
            ...
            <div className="relative">
              <div className="aspect-[4/3] bg-white rounded-3xl shadow-2xl border border-gray-100 p-5 flex flex-col relative overflow-hidden">
                ...
                <div className="flex gap-3 flex-1 min-h-0">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 flex-1">
                      {[
                        ...
                        <div key={slot.id} className={`rounded-lg p-1.5 border text-center flex flex-col items-center justify-center ${slot.occupied ? 'bg-indigo-50 border-indigo-200' : 'bg-gray-50 border-gray-200 border-dashed'}`}>
                          ...
                          {slot.occupied
                            ? <>
                                <div className="w-8 h-4 bg-indigo-400 rounded-sm mt-0.5 mb-0.5" />
                                <div className="text-[8px] font-bold text-indigo-700 truncate w-full text-center">{slot.plate}</div>
                              </>
                  ...
                  <div className="w-28 flex flex-col gap-2">
                    ...
                    <div className="bg-gray-900 rounded-xl p-2.5 flex flex-col items-center">
                      ...
                      <div className="relative w-full bg-gray-800 rounded-lg h-12 flex items-center justify-center overflow-hidden">
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="border border-indigo-400 rounded w-14 h-6 flex items-center justify-center">
                            <div className="text-[8px] text-indigo-300 font-mono">B 9876 ZZ</div>
                          </div>
                        </div>
                    ...
                    {/* Check-in queue */}
                    <div className="bg-gray-50 rounded-xl p-2.5 border border-gray-100 flex-1">
                      <div className="text-[9px] font-bold text-gray-500 mb-1.5">Masuk Antrean</div>
                      {[
                        { name: 'Pak Budi', time: '14:02' },
                        { name: 'Ibu Rina', time: '14:08' },
                      ].map((c, i) => (
                        <div key={i} className="flex items-center gap-1.5 mb-1.5">
                          <div className="w-4 h-4 rounded-full bg-indigo-200 text-indigo-700 text-[8px] flex items-center justify-center font-bold shrink-0">{c.name[4]}</div>
                          <div className="min-w-0">
                            <div className="text-[9px] text-gray-700 font-semibold truncate">{c.name}</div>
                            <div className="text-[8px] text-gray-400">{c.time}</div>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {/* Floating badge */}
              <div className="absolute -right-4 -bottom-4 bg-white p-3 rounded-2xl shadow-xl border border-gray-100 flex items-center gap-2 animate-[bounce_3.5s_infinite]">
                <div className="bg-indigo-100 text-indigo-600 p-1.5 rounded-lg">
                  <SmartphoneNfc size={14} />
                </div>
                <div>
                  <div className="text-[10px] text-gray-400">Plat Terdeteksi</div>
                  <div className="text-xs font-bold">B 9876 ZZ ✓</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="max-w-6xl mx-auto px-4 mb-24">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <h3 className="font-bold text-lg mb-3">Scan Plat & Kendaraan AI</h3>
              <p className="text-gray-600 text-sm leading-relaxed">
                Ambil foto kendaraan/plat. Sistem bantu isi data tanpa ribet — jauh lebih cepat untuk operasional di lapangan.
              </p>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <h3 className="font-bold text-lg mb-3">Contoh Ticket via WhatsApp</h3>
              <p className="text-gray-600 text-sm leading-relaxed">
                Tidak ada lagi tiket kertas yang hilang. Tiket valet digital otomatis terkirim ke WhatsApp pelanggan.
              </p>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <h3 className="font-bold text-lg mb-3">Laporan & Audit Aktivitas</h3>
              <p className="text-gray-600 text-sm leading-relaxed">
                Pantau jumlah kendaraan masuk, keluar, laporan arus kendaraan, tarif fleksibel, dan total pendapatan harian.
              </p>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <h3 className="font-bold text-lg mb-3">Presensi & Pengambilan</h3>
              <p className="text-gray-600 text-sm leading-relaxed">
                Fitur presensi staf built-in, serta kemudahan pelanggan meminta kendaraan diambilkan secara online.
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="pricing" className="max-w-5xl mx-auto px-4 mb-24">
          ...
          {/* Voucher promo from CMS */}
          {pricingData && pricingData.promoCodes && pricingData.promoCodes.length > 0 && (
            <div className="max-w-xl mx-auto mb-10">
              <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 mb-4 text-center">
                <div className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-1">Khusus bulan Juni</div>
                <div className="font-bold text-gray-700 mb-1">Kupon Promo Paket Bulanan</div>
              ...
              />
              <p className="text-xs text-gray-400 text-center mt-2">Promo voucher: minimal pembayaran 6 bulan.</p>
            </div>
          )}

          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {/* Paket Bulanan All-in */}
            <div className="bg-white rounded-3xl p-8 border-2 border-indigo-500 shadow-xl shadow-indigo-500/10 flex flex-col">
              ...
              <div className="my-5 pb-5 border-b border-gray-100">
                {promoDiscount > 0 && (
                  <div className="text-sm text-gray-400 line-through mb-0.5">{formatPrice(999000)}/bulan</div>
                )}
                <div className={`text-4xl font-black ${promoDiscount > 0 ? 'text-green-600' : 'text-indigo-600'}`}>
                  {formatPrice(999000 * (1 - promoDiscount / 100))}
                  <span className="text-base font-semibold text-gray-400"> / bulan</span>
                </div>
                {promoDiscount > 0 && <div className="text-xs text-green-600 font-bold mt-1">Diskon {promoDiscount}% aktif ✓</div>}
              </div>
              <ul className="flex-1 space-y-3 mb-8">
                {[
                  'Valet & Parkir (tiket masuk, checkout, invoice)',
                  'Presensi staf & histori transaksi',
                  'Branding venue (nama, alamat, logo)',
                  'Laporan operasional + reprint + e-ticket',
                ].map((f, i) => (
              ...
              <button
                onClick={() => handleConsultation(`Valet All-In — ${formatPrice(999000 * (1 - promoDiscount / 100))}/bulan`)}
                className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-600/30"
              >
            ...
            <div className="bg-gray-900 rounded-3xl p-8 border border-gray-800 flex flex-col">
              ...
              <ul className="flex-1 space-y-3 mb-8">
                {[
                  'Harga spesial untuk 2+ lokasi (penawaran korporat)',
                  'Branding khusus + kebutuhan operasional venue',
                  'Proses berlangganan & pembayaran via WhatsApp resmi (088 909 777 888)',
                ].map((f, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-sm text-gray-400">
                    <div className="w-4 h-4 rounded-full bg-gray-700 text-gray-300 flex items-center justify-center shrink-0 mt-0.5">
                      <svg width="8" height="6" viewBox="0 0 8 6" fill="none"><path d="M1 3l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </div>
                    {f}
                  </li>
              ...
              <button
                onClick={() => handleConsultation('Custom / Multi Lokasi Valet Indonesia')}
                className="w-full bg-white/10 hover:bg-white/20 text-white border border-white/20 py-4 rounded-xl font-bold transition"
              >
                Konsultasi via WhatsApp
              </button>
            </div>
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400 mt-1">Dapatkan penawaran harga dan potongan kupon khusus yang tidak tersedia jika Anda mendaftar sendiri.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <ShieldCheck className="text-indigo-400 shrink-0 mt-1" size={20} />
                    <div>
                      <h3 className="font-bold text-lg">Aftersales Prioritas</h3>
                      <p className="text-sm text-gray-400 mt-1">Bantuan dari proses setup awal hingga training staf valet Anda akan dibantu langsung oleh tim LOKAL.</p>
                    </div>
                  </div>
                </div>
              </div>
